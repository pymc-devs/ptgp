...
def test_positive_propagates_through_mul_only_if_all_positive():
    x = assume(pt.dscalar("x"), positive=True)
    y = assume(pt.dscalar("y"), positive=True)
...
def test_positive_propagates_through_dimshuffle():
    x = assume(pt.dvector("x"), positive=True)
...
def test_alloc_diag_of_symbolic_positive_vector_is_psd():
    v = assume(pt.dvector("v"), positive=True)
    M = pt.diag(v)
...
def test_mul_of_positive_scalar_and_psd_matrix_is_psd():
    sigma = assume(pt.dscalar("sigma"), positive=True)
    K = assume(pt.dmatrix("K"), positive_definite=True, symmetric=True)
    M = sigma**2 * K
...
def test_quadratic_form_dot_is_psd():
    ...
    X = pt.dmatrix("X")
    M = assume(pt.dmatrix("M"), positive_definite=True, symmetric=True)
    Q = X.T.dot(M.dot(X))
...
def test_slogdet_of_LLT_takes_diag_shortcut_when_L_lower_triangular():
    ...
    L = assume(pt.dmatrix("L"), lower_triangular=True)
